...
//#include "CayleySpaceView.h"	replace after update
//#include "SweepView.h"
...
class DistinctRealization {
 public:
  static vector<double> cal_di(MolecularUnit *hel, PredefinedInteractions *df,
  ...
  static string cal_flip_string(MolecularUnit *hel, PredefinedInteractions *df,
                                const DistinctRealization &r) {
    // vector<int> res;
    stringstream res;
    vector<Atom *> atoms;
    ...
    for (PredefinedInteractions::dist_iterator dit = df->dist2begin();
      ...
      count++;
      Atom *atom = hel->index.at(dit->first.first);
      if (find(atoms.begin(), atoms.end(), atom) == atoms.end()) {
        atoms.push_back(atom);
        points.push_back(Vector(atom->getLocation()).trans(r.trans_mat));
      }
 ...
 public:
  ...
  DistinctRealization(MolecularUnit *hel, PredefinedInteractions *df,
                      double fa[][3], double ta[][3], double fb[][3],
                      double tb[][3], int node, int spacenum,
                      string *fstring = NULL) {
    trans_mat = Utils::getSingleTransformFromTwo(Utils::getTransMatrix(fa, ta),
                                                 Utils::getTransMatrix(fb, tb));
    this->node = node;
...
class RealizationCompare {
 ...
 public:
  RealizationCompare(MolecularUnit *hel, int threshhold = 3) {
    this->hel = hel;
  ...
  double difference(const DistinctRealization &r1,
    ...
    double max_dis = 0;
#if 0
        for(map<string, Atom*>::iterator iter = hel->index.begin();
  ...
  bool diff_flip(const DistinctRealization &r1,
    ...
    for (int i = 0; i < r1.flip_string.size(); i++) {
      if (r1.flip_string[i] != r2.flip_string[i]) return false;
    }
    return true;
  }
...
class RealizationHash {
 ...
 public:
  RealizationHash(MolecularUnit *hel, DistinctRealization &re)
      : refer(re), rc(hel) {
    this->hel = hel;
  }
...
class RatioChecker {
 public:
  RatioChecker(SaveLoader *snl, MolecularUnit *hA, MolecularUnit *hB,
               Atlas *mapview);
