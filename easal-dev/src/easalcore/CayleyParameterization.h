...
/*
 * This class chooses non-edges in an ActiveConstraintGraph that complete the
 * graph into 3-tree. Those non-edges are called the parameters. The complexity
 * of the sampling algorithm varies based on the choice of non-edges and the
 * order in which they are fixed.
 */
...
class CayleyParameterization {
 public:
   ...
   * It calls the helper methods to determine the parameters that make the graph
   * 3-tree
   *
   * @param basic True keeps the parameters unordered to fasten the process,
   * False built3tree to order parameters.
   */
   ...
   * i.e. set of parameters, each represented by <helix A "vertex" index , helix
   * B "vertex" index>
   */
  ...
  /**
   * @brief Finding bounds for each non-edge is either solving a linear
   * inequality or solving a single quadratic of one variable.
   *
   ...
   * 3-realizable (3-tree) extension by leveraging the convex parametrization
   * theory.
   *
   * @detail Checks if the contacts of currentGraph is subset of any
   * isomorphisms of the graphs in the list of complete3trees If found then the
   * edges in that graph (except contacts) will be used as parameter set. If the
   * contacts are not part of any pre-determined graphs i.e. it is not partial
   * 3-tree, then assign random parameters
   *
   * @return Set of parameters each represented by <helix A "vertex" index ,
   * helix B "vertex" index>
   */
  virtual vector<pair<int, int> > defineParameters();

  /**
   * @brief An alternative method to pick the parameters for 5 dimensional
   * region such that the range of each parameter has similar length. It aims
   * sampling more uniformly on Cartesian space by setting Cayley parameter
   * space as spheric as possible.
   *
   * @detail Set parameters such that the deviation of parameter intervals is
   * minimal i.e. choose a parameter set that will give almost a "spheric"
   * parameter space. note that sampling occurs from min to max values of a
   * parameter step by step.
   * ---
   * for 5d case, we have 1 contact and need 5 parameters. There are 8 non-edges
   * in contact graph. we need to eliminate 3 non-edges to find that set: order
   * those 8 non-edges according to length of intervals. The set of non-edges in
   * the middle will give the parameters with min deviation. So eliminate the 3
   * non-edges from top or down such that remaining 5 non-edges will be our
   * parameter set with the min deviation.
   *
   * @return Set of parameters each represented by <helix A "vertex" index ,
   * helix B "vertex" index>
   *
  ...
  /**
   * @brief 3-tree formed by starting with a 4-vertex complete graph and then
   * repeatedly adding vertices in such a way that each added vertex has exactly
   * 3 neighbors that form a clique.
   *
   * Finds a 4 clique as a base tetrahedra. Then complete 3-tree is
   * built up from a base tethedra by adding, at each step, a new vertex
   * edge-connected to the face of a tetrahedra. Store the tetrahedras in the
   * order they are created in attribute tetrahedra.
   *
   ...
   * And create a new tetrahedron with this vertex plus the 3 vertices of the
   * face that it is connected to. Then add this tetrahedron to tetrahedra
   * array.  HERE ADD THE LABEL FOR MISSING EDGE ???!!!
   *
   * @return The index of vertex that is just identified to be known by location
   * wise
   *
   * previous name was findPointThatIsConnectedToFaceOfTetraHedra  or
   * findPointsAcordingToKnownLocations
   */
  int identifyRigidVertex();

  /**
   * @brief Search for any undefined edges connected to the vertex if the
   * neighbor vertex is also location-wise known. Then updates this edge to be
   * True(known)
   *
   ...
   *
   * The parameter in the first tetrahedra will be at the end, if the parameter
   * takes place in the same tetrahedra, then max degree will be put to the end
   *
   ...
   * till paramss empty.
   *
   * so this algorithm orders the parameters in the reverse order they take
   * place in the tetrahedras. if there are multiple parameters in one
   * tetrahedra, then the parameter with higher degree will be put to the end
   * side
   */
 ...
 private:
   ...
   * that will be important later during sampling procedure. This data is later
   * on passed to ConvexChart and CartesianRealizer to help their computations.
   *
  ...
  /**
   * shows if the length of the edge between vertices of ActiveConstraintGraph
   * is known or not
   *
  ...
  /**
   * shows if the length of the edge is known by the vertices coordinates that
   * it is connected to.
   *
   * i th vertex from helix B is represented by i+NO/2 as index number
   */
   ...
   * the index of the tetrahedron in tetrahedra if tetrahedral inequality is
   * used to compute the range of the parameter
   */
