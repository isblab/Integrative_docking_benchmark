...
class Atom {
 public:
  ...
  Atom(double location[], double radius);

  /** @brief Constructor with location, radius, name, and atomID initialization
   */
  Atom(double location[], double radius, std::string name, std::string atomID);
   ...
   */
  void setAtomID(std::string atomID);
  ...
  /**
   * @brief Convert the "data" that was associated with the atom to the
   * pdb_format style
