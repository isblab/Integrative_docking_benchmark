...
/**
 * This class contains routines that computes Orientation that represents
 * transformations of the rigid molecular composite s relative to each other. It
 * computes Cartesian realization of an ACG with the parameter lengths taken
 * from cayleyPoint ? and active constraint lengths for a specific flip. It
 * intentionally ignores the remainder of the assembly constraint system, namely
 * atom markers not in G and their constraints.
 */
class CartesianRealizer {
 public:
  ...
  // ConvexChart* descr, int flipno, bool& fail); static Orientation*
  // computeRealization(ActiveConstraintGraph* cgk, ConvexChart* descr, int
  // flipno,  double paramconnections[][12], bool& fail ) ;

  static Orientation* computeRealization(ActiveConstraintGraph* cgk,
                                         ConvexChart* descr, int flipno,
                                         bool& fail,
                                         vector<vector<int> > flipScheme);
  static Orientation* computeRealization(ActiveConstraintGraph* cgk,
                                         ConvexChart* descr, int flipno,
                                         double paramconnections[][12],
                                         bool& fail,
                                         vector<vector<int> > flipScheme);

 private:
  /** @brief clean toA and toB members of the class */
  static void destructor();  // Not needed

  /** @brief Checks if the transformation from fromA to toA corrupts or not*/
  static bool isDistorted();  // Copy this function over

  /** @brief Creates a new Orientation with fromB, toB and flip initialization
   */
  static Orientation*
  getOrientation();  // Can be copied over. Not really needed.
   ...
   */
  static bool setBaseTetra(
      bool mirror);  // Will take positions as a reference and populate it
   ...
   */
  static void computeLengthOfTheEdgesConnectedToVertex(
      int vertex);  // Will take positions as a reference and use it
   ...
   */
  static bool locateVertex(
      int tetrahedron_index,
      bool mirror);  // Will take positions as an argument and use it
   ...
   */
  static void locateTetrahedron(
      int tetrahedron_index, double positions[][3],
      bool mirror);  // Will take position as an argument

  /** @brief Computes the translation and Euler angles needed for Jacobian
   * computation */
  static void compute_TB_EAB();  // Most things needed are available in Settings

  /** @brief Print Cartesian location of indices and the lengths of edges in
   * between those vertices for debug purpose*/
  static void printPositionsAndConnections(
      vector<int> ind);  // Get rid of this.
  ...
  static bool partial3tree;  // Copied from ConvexChart
  ...
  static std::vector<std::vector<int> >
      tetrahedra;  // Copied from convex chart / flipscheme

  static Eigen::Vector3d TB;   // Computed On the fly
  static Eigen::Vector3d eaB;  // Computed On the fly

  static bool debug;  // Shouldn't be a var.

  /** false if volume is negative */
  static bool realizable;  // Computed on the fly
  // todo check if volume computations are accurate enough and check the error
  // messages printed to the console

  static MolecularUnit *helA, *helB;  // Should be moved to settings.
   ...
   * graph
   */
  static std::vector<int> verticesA;  // Get this from ACG
  static std::vector<int> verticesB;  // Get this from ACG

  static ActiveConstraintGraph* graph;  // Input in computeRealization

  /**
   * For 3*3 graphs there are 3 tetrahedrons
   * For 3*4 graphs there are 4 tetrahedrons. One of the tetrahedron is fixed.
   * (The one where all the vertices located in one helix) For 3*5 graphs there
   * are 5 tetrahedrons. Two of the tetrahedrons are fixed. (The ones where all
   * the vertices located in one helix) For 3*6 graphs there are 6 tetrahedrons.
   * Three of the tetrahedrons are fixed. (The ones where all the vertices
   * located in one helix)
   *
   * there are 3 tetrahedrons hence 2^3=8 flips.
   *  If all vertices of the tetrahedron is in the same rigid molecule, then it
   * does not contribute to the flip set. todo checks the effects of this
   * situation
   *
   *  for 2 helices, there are only 3 tetrahedrons satisfying above and being
   * partial 3-tree graph.
   */
  static int flipNo;  // Input to computeRealization
  ...
  /** All fixed distances plus current distance values of non-edges of
   * ActiveConstraintGraph previous name was pconnections
   */
