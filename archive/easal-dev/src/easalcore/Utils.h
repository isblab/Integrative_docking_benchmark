...
class Vector3 {
 public:
  ...
  double& operator[](int idx) { return coord[idx]; }

  double* getLoc() { return coord; }
...
class Utils {
 public:
  ...
  static int findPair(vector<pair<int, int> > myvector, pair<int, int> mypair);
  ...
  static double tetrahedralVolumeUsingCayleyMengerDeterminant(double* len);
   ...
   *
   * This method sometimes give negative value (may be huge neg value)  when
   volume is close to zero. It cannot handle critic cases
   *
   * @param len 6dimensional array that holds the lengths of edges of the
   tetrahedron
   */
   ...
   */
  static double* matApp2(double* x, double* a, double* b, double* c, double* ap,
                         double* bp, double* cp);
  ...
  /**
   * @brief Computes the location of a point that lies in the same plane with a
   * b and c
   * @param a b c  are the locations
   * @param ap bp cp are the lengths of the edges from unknown point x to those
   * 3 points a b c
  ...
  /**
   * @brief This method takes a set of 6 lengths and outputs the locations of
   * the 4 points
   *
   * @param lengths 6dimensional array that holds the lengths of edges of the
   * tetrahedron
  ...
  /**
   * @brief This method returns a matrix that is combination of rotation and
   * translation.
   *
  ...
  /**
   * @param ta Transformation matrix that is combination of rotation and
   * translation.
   * @param tb Transformation matrix that is combination of rotation and
   * translation.
   ...
   * @param tb Transformation matrix that is combination of rotation and
   * translation.
   * @return Transformation matrix that is combination of ta and tb (First apply
   * a and then b)
   */
   ...
   * translation.
   *
   * @param filename The path of the file that holds original Cartesian
   * coordinates of three points and destination Cartesian coordinates of three
   * points
   *
   * @see getTransMatrix
   */
