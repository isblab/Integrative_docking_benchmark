 ...
 * It is designed to check whether any non-active constraints become active or
 * whether they are violated.
 *
 * User have the option to define set of constraints of interest. In that case,
 * new constraint activation check will be done only for this set. For an input
 * Orientation, ConstraintCheck first computes the Cartesian realization for the
 * entire molecular composite then passes it to the subroutines to do user
 * specified constraint check such as steric constraints, tethering constraints
 * or angle constraints.
 */
class ConstraintCheck {
 public:
  /** @brief Constructor with ActiveConstraintGraph and PredefinedInteractions
   * initialization */
  ...
  /** @return True if the atoms a and b intersects more than the allowed
   * threshold, False otherwise */
  bool stericViolated(Atom* a, Atom* b);
   ...
   */
  void updatePossibleContactList(Atom* a, Atom* b, int indexA, int indexB);

  /**
   * @brief Computed the transformation between helix A and helix B
   * Assumes initially helixA and helixB were aligned paralel
   * Hence instead of computing the angle between fixed helixA and oriented
   * helixB, computing the angle between initial helixB and oriented helixB,
   * would do the same job.
   *
   * @param fb, tb Determines transformation matrix from original helixB to the
   * oriented helixB
   *
   * @return True if the angle between two helices is out of the allowed range,
   * False otherwise
   */
  bool angleViolationCheck(double fb[3][3], double tb[3][3]);

  /*
   * @brief This function checks if steric constraint is violated for atom pairs
   * between helixA and helixB Updates the angle flag of the orientation
  ...
  bool outerGrid;  // for Marias case: to check if there is a pair close enough
                   // or not
