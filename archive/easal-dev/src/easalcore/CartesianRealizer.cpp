...
MolecularUnit* CartesianRealizer::helA;
MolecularUnit* CartesianRealizer::helB;
...
Orientation* CartesianRealizer::computeRealization(ActiveConstraintGraph* cgk,
ConvexChart* descr, int flipno,  double paramconnections[][12],  bool& fail  ) {
        fail = false;
        ...
        for(int i= 0;i<NO;i++){
                positions[i][0]= -111;	positions[i][1]= -111; positions[i][2]=
-111;
        }
        ...
        if( partial3tree){
                ...
                if( setBaseTetra( mirrorlist[0] ) )  //if indices are in the
same helix , then no need for mirror mirrorIndex = 0; if( !realizable ){ fail =
true; destructor(); return NULL;
                }

                //builds new locations onto the base tetrahedron.
                size_t  tetraIndex = 1;
                do{
                        if( !locateVertex( tetraIndex, mirrorlist[mirrorIndex] )
) //if locateVertex is true, then indices are in the same helix, then no need
for mirror and volume is positive
                        {
                                if( !realizable ){
                                        fail = true;
                                        destructor();
...
tetrahedra[tetraIndex][0] ); mirrorIndex++;

                        }
                        tetraIndex++;
                }while( tetraIndex < tetrahedra.size() );
                //now it should be able to fill all the values of vertices, else
it means it needs maple
...
std::isnan(toA[i][j])  || std::isnan(toB[i][j]) ) cout << "eroorrrr toA[i][j]==
-111 || toB[i][j]== -111" << endl;
                        }
                }
                ...
                for(int j=0; j<3; j++){ //tranlates toB[j]
                        double pot[3];
                        Utils::matApp(toB[j] ,toA[0], toA[1], toA[2], fromA[0],
fromA[1], fromA[2], pot);
        ...
        if( settings::Sampling::JacobianSampling )
                compute_TB_EAB();
...
Orientation* CartesianRealizer::computeRealization(
    ActiveConstraintGraph* cgk, ConvexChart* descr, int flipno, bool& fail,
...
Orientation* CartesianRealizer::computeRealization(
    ActiveConstraintGraph* cgk, ConvexChart* descr, int flipno,
    double paramconnections[][12], bool& fail,
    vector<vector<int> > flipScheme) {
  ...
  helA = sett->runTimeObjects.muA;
  helB = sett->runTimeObjects.muB;
  ...
  partial3tree = descr->partial3tree;
  if (partial3tree) {
    // TODO: wtf seg fault here?????? - BRIAN
    tetrahedra = descr->getTetras();
  } else {
    tetrahedra = flipScheme;
  }

  size_t mirrorIndex = 1;
  if (setBaseTetra(mirrorlist[0]))  // if indices are in the same helix , then
                                    // no need for mirror
    mirrorIndex = 0;
  if (!realizable) {
    fail = true;
  ...
  do {
    if (!locateVertex(
            tetraIndex,
            mirrorlist[mirrorIndex]))  // if locateVertex is true, then indices
                                       // are in the same helix, then no need for
                                       // mirror and volume is positive
    {
      if (!realizable) {
        fail = true;
        destructor();
  ...
  // FIX HELIX A
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++) toA[i][j] = fromA[i][j];

  // end original if
  //}

  if (sett->Sampling.JacobianSampling) compute_TB_EAB();

  Orientation* ori_output = getOrientation();
  destructor();
  return ori_output;
}
...
bool CartesianRealizer::locateVertex(int tetrahedron_index, bool mirror) {
      ...
      ind[3] < NO / 2)  // inside of same helix i.e. helix A
  {
    ...
    for (int i = 0; i < 3; i++) positions[ind[0]][i] = pot[i];
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (debug) cout << positions[ind[0]][i] << endl;
  }
  ...
  if (debug)
    cout << "fact " << edge_length[ind[0]][ind[1]] << " "
         << edge_length[ind[0]][ind[2]] << " " << edge_length[ind[0]][ind[3]]
         << endl;
  if (debug)
  ...
  if (disturbed || foundNan) {
    if (debug)
      for (int i = 1; i < 4; i++) {
        ...
        for (int j = i + 1; j < 4; j++)
          cout << ind[i] << "-" << ind[j] << ": " << edge_length[ind[i]][ind[j]]
    ...
    if (abs(edge_length[ind[0]][ind[1]] -
            Utils::dist(positions[ind[0]], positions[ind[1]])) > .6)
      realizable = false;
    if (abs(edge_length[ind[0]][ind[2]] -
            Utils::dist(positions[ind[0]], positions[ind[2]])) > .6)
      realizable = false;
    if (abs(edge_length[ind[0]][ind[3]] -
...
void CartesianRealizer::computeLengthOfTheEdgesConnectedToVertex(int vertex) {
  for (int i = 0; i < NO; i++)
    if (positions[i][0] != -111 &&
        edge_length[i][vertex] ==
            -1)  // if ith vertex is located before and if the distance i-vertex
                 // is not defined before
    {
      edge_length[i][vertex] = edge_length[vertex][i] =
...
void CartesianRealizer::locateTetrahedron(int tetrahedron_index,
  ...
  for (int i = 0; i < 4; i++)
    for (int j = i + 1; j < 4; j++)
      if (abs(edge_length[inds[i]][inds[j]] - Utils::dist(posi[i], posi[j])) >
          .6) {
        disturbed = true;
      }
  ...
  if ((vc < 15 && vc >= 0 && disturbed) || foundNan) {
    ...
    double ac = edge_length[inds[0]][inds[2]],
           ab = edge_length[inds[0]][inds[1]],
           bc = edge_length[inds[1]][inds[2]];
...
Orientation* CartesianRealizer::getOrientation() {
  Settings* sett = Settings::getInstance();
