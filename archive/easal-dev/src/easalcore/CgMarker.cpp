...
void CgMarker::mark(AtlasNode* rnode) {
  ...
  Orientation* orr =
      new Orientation(region->getSpace().at(0)->getOrientations().at(0));
  int ref_count = 0;
  ...
  for (int i = 0; i < ancestors.size(); i++) {
    ...
    if (cgchildren.find(ancestors[i]) ==
        cgchildren.end()) {  // initial the children array for newly added cg
      cgchildren[ancestors[i]] =
    ...
    for (int j = i + 1; j < ancestors.size(); j++) {  // for each ancestors
      ...
      if (parent != -1 && child != -1) {
        ...
        for (vector<Edge>::iterator eit = children.begin();
          ...
          if (eit->first == ancestors[child]) count++;
          if (count >= 20) break;
        }
...
void CgMarker::try_release_orientation(Orientation* orr) {
  // decrease the reference counter of the Orientation. Free it if the counter
  // is 0.
...
bool CgMarker::empty() { return cgs.empty(); }
...
void CgMarker::report_size() {
  cout << "cgs size:" << cgs.size() << endl;
  cout << "cgchildren size:" << cgchildren.size() << endl;
}
