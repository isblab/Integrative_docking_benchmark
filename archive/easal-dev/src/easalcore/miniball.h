...
double Miniball<d>::max_excess(It t, It i, It& pivot) const {
  ...
  for (It k = t; k != i; ++k) {
    ...
    e = -sqr_r;
    for (int j = 0; j < d; ++j) e += mb_sqr(p[j] - c[j]);
    if (e > max_e) {
      max_e = e;
      pivot = k;
    }
...
double Miniball_b<d>::excess(const Point<d>& p) const {
  ...
  for (int k = 0; k < d; ++k) e += mb_sqr(p[k] - current_c[k]);
  return e;
}
...
bool Miniball_b<d>::push(const Point<d>& p) {
  ...
  double eps = 1e-32;
  if (m == 0) {
    for (i = 0; i < d; ++i) q0[i] = p[i];
    for (i = 0; i < d; ++i) c[0][i] = q0[i];
    sqr_r[0] = 0;
  } else {
    // set v_m to Q_m
    ...
    for (i = 1; i < m; ++i) {
      for (j = 0; j < d; ++j) v[m][j] -= a[m][i] * v[i][j];
    }

    // compute z_m
    z[m] = 0;
    for (j = 0; j < d; ++j) z[m] += mb_sqr(v[m][j]);
    z[m] *= 2;
    ...
    // update c, sqr_r
    double e = -sqr_r[m - 1];
    for (i = 0; i < d; ++i) e += mb_sqr(p[i] - c[m - 1][i]);
    f[m] = e / z[m];
