...
ActiveConstraintGraph::ActiveConstraintGraph(ActiveConstraintGraph* ptrToCpy) {
  for (vector<pair<int, int> >::iterator iter = ptrToCpy->participants.begin();
       iter != ptrToCpy->participants.end();
       iter++) {  // why not just copy all from prtToCpy
  ...
  // this->setParticipants(ptrToCpy->getParticipants());
  // this->set_unordered_setA(ptrToCpy->get_unordered_setA());
  // this->set_unordered_setB(ptrToCpy->get_unordered_setB());
  /*
          this->setParameters(ptrToCpy->getParameters());
          this->setContacts(ptrToCpy->getContacts());
          this->setParamLines(ptrToCpy->getParamLines());
  */
  this->setStepSize(ptrToCpy->getStepSize());
  // this->setVerticesA(ptrToCpy->getVerticesA());
  // this->setVerticesB(ptrToCpy->getVerticesB());

  completeTo3by3Graph();  // this method will be executed again when a new
                          // contact is added later on. since verticesA/B
                          // includes vertex for the parameters as well, those
                          // vertices will be overwritten by future contact
                          // atoms.
}
...
ActiveConstraintGraph::ActiveConstraintGraph(
  ...
  this->stepSize = sett->Sampling.stepSize;
  ...
  //	this->unordered_setX.unique(); //unique does not work since it removes
  //'consecutive' equal elements. Hence you need to check existence before
  //adding as it is done above.

  // if( !settings::Statistics::createPseudoAtlas)
  if (!sett->Statistics
           .createPseudoAtlas)  // during createPseudoAtlas, contactGraph is
                                // created for every config. It is too
                                // inefficient and takes a lot of time to create
                                // parameters everytime. I am assuming, I do
                                // import RoadMap.txt file which is the list of
                                // graph with parameters from previous easal
                                // atlas generations.
  {
    completeTo3by3Graph();
  }
...
ActiveConstraintGraph::ActiveConstraintGraph(
    vector<pair<int, int> >& participants, vector<pair<int, int> > PL) {
  Settings* sett = Settings::getInstance();
  this->stepSize = sett->Sampling.stepSize;
  ...
  // the vertex that 'first' appear in PL should be added to verticesA/B
  // 'first'.
...
ActiveConstraintGraph::~ActiveConstraintGraph() {
  this->unordered_setA.clear();
  this->unordered_setB.clear();
...
void ActiveConstraintGraph::completeTo3by3Graph() {
  ...
  if (this->unordered_setA.size() <= 2)  // find missing vertices
  {
    vector<int> toAdd;
    if (closestAtom)
      toAdd = completeTo3by3Graph_byClosestAtom(sett->runTimeObjects.muA,
                                                this->verticesA,
                                                this->unordered_setA.size());
    else  // perpendicular angle
      toAdd = completeTo3by3Graph_that_makes_perpendicularAngle(
          sett->runTimeObjects.muA, this->verticesA,
          this->unordered_setA.size());

    for (int i = 0; i < toAdd.size(); i++) this->verticesA.push_back(toAdd[i]);
  }
  ...
  if (this->unordered_setB.size() <= 2)  // find missing vertices
  {
    vector<int> toAdd;
    if (closestAtom)
      toAdd = completeTo3by3Graph_byClosestAtom(sett->runTimeObjects.muB,
                                                this->verticesB,
                                                this->unordered_setB.size());
    else  // perpendicular angle
      toAdd = completeTo3by3Graph_that_makes_perpendicularAngle(
          sett->runTimeObjects.muB, this->verticesB,
          this->unordered_setB.size());

    for (int i = 0; i < toAdd.size(); i++) this->verticesB.push_back(toAdd[i]);
  }
}
...
vector<int> ActiveConstraintGraph::completeTo3by3Graph_byClosestAtom(
  ...
  if (size == 1)  // Find 2nd atom that is closest to first atom
  {
    vertices_1 =
        vertices[0] +
        1;  // no need to worry about duplicates since there was only 1 atom.
    if (vertices_1 >= hel_size) vertices_1 = vertices[0] - 1;
  ...
  do {
    h2 = i++;
  } while ((vertices[0] == h2 || vertices_1 == h2) &&
           i < hel_size);  // while h2 already exists in current vertices  and
                           // there is more atoms to search for
...
// todo actually it would be better to choose equilateral triangle. because in
// perpendicular case, 2 atoms can be so close to each other.
vector<int>
ActiveConstraintGraph::completeTo3by3Graph_that_makes_perpendicularAngle(
    MolecularUnit* hel, vector<int> vertices, int size) {
  vector<int> out_vertices;
  ...
  if (size == 1)  // Find 2nd atom
  {
    ...
    for (int h = 0; h < hel_size; h++) {
      if (h == vertices[0]) continue;
      double dist = Utils::dist(hel->getAtomAt(vertices[0])->getLocation(),
    ...
    if (vertices_1 == -1)
      cout << "ERROR in completeTo3by3Graph_that_makes_perpendicularAngle "
           << endl;
  ...
  for (int h = 0; h < hel_size; h++) {
    if (vertices[0] == h ||
        vertices_1 == h)  // if h already exists in current vertices
      continue;
    ...
    double dist_1h = Utils::dist(hel->getAtomAt(vertices[1])->getLocation(),
                                 hel->getAtomAt(h)->getLocation());
    double angle =
        acos((dist_0h * dist_0h + dist_1h * dist_1h - dist_01 * dist_01) /
             (2 * dist_0h * dist_1h)) *
        180.0 / PI;
...
int ActiveConstraintGraph::getB(int no) {
  if (this->verticesB.size() > no)
    ...
    cout << "getB ERROR" << endl
         << no << " this->verticesB.size() " << this->verticesB.size() << endl;
    return -1;
  }
}
...
int ActiveConstraintGraph::verticesA_size() { return this->verticesA.size(); }

int ActiveConstraintGraph::verticesB_size() { return this->verticesB.size(); }
...
bool ActiveConstraintGraph::mayHaveThirdAtom() {
  if (this->participants.size() < 6 ||
      (this->unordered_setA.size() > 2 && this->unordered_setB.size() > 2))
    return true;
  return false;
}
...
bool ActiveConstraintGraph::isDependent() {
  for (int i = 0; i < this->participants.size(); i++) {
    ...
    for (int j = i + 1; j < this->participants.size(); j++)
      if (this->participants[i].first == this->participants[j].first) count++;
  ...
  for (int i = 0; i < this->participants.size(); i++) {
    int count = 1;
    for (int j = i + 1; j < this->participants.size(); j++)
      if (this->participants[i].second == this->participants[j].second) count++;
    if (count >= 4) return true;
  }
...
size_t ActiveConstraintGraph::getParamDim() { return paramlines.size(); }
...
void ActiveConstraintGraph::addContact(pair<int, int> contact) {
  ...
  for (vector<pair<int, int> >::iterator iter = this->participants.begin();
       iter != this->participants.end(); iter++)
    if (((*iter).first == contact.first &&
         (*iter).second == contact.second))  // that contact exists before
      contactExisted = true;
  ...
  if (find(this->unordered_setA.begin(), this->unordered_setA.end(),
           contact.first) ==
      this->unordered_setA.end())  // if cannot find, then push
...
bool ActiveConstraintGraph::IsParentOf(
    ActiveConstraintGraph* other) {  // if other is child of this, then it
                                     // should have more contacts
  bool output = true;
  output = output && (this->participants.size() <= other->participants.size());
  output = output && (this->getParamDim() >=
                      other->getParamDim());  // to make sure 5d is parent of 4d
                                              // even they have both 2 dumbbells
  ...
  // output = output && (this->helA == other->getMolecularUnitA() ); //make it
  // slow?  // i put this here from top, to make it not checked if participants
  // do not match output = output && (this->helB == other->getMolecularUnitB() );
  // //if it checks just pointer address then it is not slow
  return output;
}

bool ActiveConstraintGraph::isConnected(int x, int y) {
  auto it =
      std::find(participants.begin(), participants.end(), make_pair(x, y));
  if (it != participants.end()) return true;
  return false;
}
...
vector<int> ActiveConstraintGraph::getVerticesA() { return this->verticesA; }
...
vector<int> ActiveConstraintGraph::getVerticesB() { return this->verticesB; }

void ActiveConstraintGraph::set_unordered_setA(list<int> a) {
  this->unordered_setA = a;
}
...
void ActiveConstraintGraph::set_unordered_setB(list<int> b) {
  this->unordered_setB = b;
}
...
/*
MolecularUnit* ActiveConstraintGraph::getMolecularUnitA(){
        return this->helA;
}

MolecularUnit* ActiveConstraintGraph::getMolecularUnitB(){
        return this->helB;
}

void ActiveConstraintGraph::setMolecularUnitA(MolecularUnit*a) {
    this->helA = a;
}

void ActiveConstraintGraph::setMolecularUnitB(MolecularUnit*b) {
    this->helB = b;
}
*/
void ActiveConstraintGraph::setContacts(
    std::vector<std::pair<int, int> > cont) {
  this->contacts = cont;
}

void ActiveConstraintGraph::setParticipants(
...
ostream& operator<<(ostream& os, ActiveConstraintGraph& acg) {
  ...
  /*
          ...
          for(list<int>::iterator iter = acg.unordered_setA.begin(); iter !=
     acg.unordered_setA.end(); iter++) os<< (*iter) << "   "; os<<endl;


          os<< "unordered_setB" << endl;
          for(list<int>::iterator iter = acg.unordered_setB.begin(); iter !=
     acg.unordered_setB.end(); iter++) os<< (*iter) << "   "; os<<endl;


          os<< "verticesA" << endl;
          for(vector<int>::iterator iter = acg.verticesA.begin(); iter !=
     acg.verticesA.end(); iter++) os<< (*iter) << "   "; os<<endl;


          os<< "verticesB" << endl;
          for(vector<int>::iterator iter = acg.verticesB.begin(); iter !=
     acg.verticesB.end(); iter++) os<< (*iter) << "   "; os<<endl;

          //////////////////////////


          os << "contacts.size " << acg.contacts.size() << " ";
          for(vector<pair<int,int> >::iterator iter = acg.contacts.begin();iter
     != acg.contacts.end();iter++) os << "c "<< iter->first << " " <<
     iter->second << " " ; os << endl;


          os << "paramlines.size " << acg.paramlines.size() << " ";
          for(vector<pair<int,int> >::iterator iter =
     acg.paramlines.begin();iter != acg.paramlines.end();iter++) os << "p "<<
     iter->first << " " << iter->second << " " ; os << endl;


          os << "parameters.size " << acg.parameters.size() << " ";
          for(vector<pair<int,int> >::iterator iter =
     acg.parameters.begin();iter != acg.parameters.end();iter++) os << "p "<<
     iter->first << " " << iter->second << " " ; os << endl;
  */
  os << endl;
...
double ActiveConstraintGraph::getStepSize() { return this->stepSize; }

void ActiveConstraintGraph::setStepSize(double siz) { this->stepSize = siz; }

int ActiveConstraintGraph::getCNoinA() { return this->unordered_setA.size(); }

int ActiveConstraintGraph::getCNoinB() { return this->unordered_setB.size(); }
...
vector<ActiveConstraintGraph*> ActiveConstraintGraph::getAllAncestors() {
  ...
  for (int i = 0; i < power_of_two; i++) {
    ...
    // if(parts.size() < 6-Settings::RootNodeCreation::dimension_of_rootNodes)
    if (parts.size() < 6 - sett->RootNodeCreation.dimension_of_rootNodes)
      continue;
    sort(parts.begin(), parts.end());
    ActiveConstraintGraph* cgnew = new ActiveConstraintGraph(parts);
    // guaranteed >= 0 since it returns unsigned (size_t)
    // if(cgnew->getDim() <=
    // Settings::RootNodeCreation::dimension_of_rootNodes){ // && cgnew->getDim()
    // >= 0
    if (cgnew->getDim() <=
        sett->RootNodeCreation
            .dimension_of_rootNodes) {  // && cgnew->getDim() >= 0
...
bool CGcomp(ActiveConstraintGraph* lhs, ActiveConstraintGraph* rhs) {
  ...
  // first compare dim
  if (lhs->getDim() != rhs->getDim()) return lhs->getDim() > rhs->getDim();
