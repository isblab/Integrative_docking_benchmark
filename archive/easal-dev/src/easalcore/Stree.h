...
class snode {
 ...
 public:
  ...
  snode(Atom *atom) : center(atom->getLocation()), radius(atom->getRadius()) {
    // self contain, careful when clean up;
    children.push_back(this);
    this->atom = atom;
  }

  snode(std::vector<Atom *> &atoms) {
    this->atom = NULL;
    ...
    miniball::Point<3> p;
    double max_rad = -1;
    for (std::vector<Atom *>::iterator iter = atoms.begin();
         iter != atoms.end(); iter++) {
      for (int i = 0; i < 3; i++) {
    ...
    while (err > 0.1 || step < 0) {  // not converge
      ...
      for (std::vector<Atom *>::iterator iter = atoms.begin();
        ...
        for (int i = 0; i < branchs; i++) {
          double dis = (v[i] - Vector((*iter)->getLocation())).squared_length();
          if (dis < mindis) {
            mindis = dis;
            minidx = i;
          }
    ...
    std::vector<std::vector<Atom *> > ps(branchs);
    for (std::vector<Atom *>::iterator iter = atoms.begin();
         iter != atoms.end(); iter++) {
      int minidx = 0;
      double mindis = 1000000.f;
...
#if 1
  bool is_leaf() { return this->atom != NULL; }
#endif
...
class Stree {
 public:
  Stree(std::vector<Atom *> &pts) : root(pts) {}
