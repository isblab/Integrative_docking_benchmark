...
/**
 * The class generate simple hash number for both ActiveConstraintGraph and
 * Orientation
 */
class CgHasher {
 public:
  size_t operator()(ActiveConstraintGraph* x) const { return size_t(x); }

  size_t operator()(Orientation* x) const { return size_t(x); }
};
...
class CgMarker {
 public:
  typedef std::pair<ActiveConstraintGraph*, Orientation*>
      Edge;  // The child node and the boundary Orientation
  typedef std::set<ActiveConstraintGraph*,
                   bool (*)(ActiveConstraintGraph*,
                            ActiveConstraintGraph*)>::iterator iterator;
 ...
 public:
  ...
  // report the size of CgMarker to monitor memory usage
  void report_size();

 private:
  // set of all contact graphs
  std::set<ActiveConstraintGraph*,
           bool (*)(ActiveConstraintGraph*, ActiveConstraintGraph*)>
      cgs;

  // the children of each contact graph(node)
  std::unordered_map<ActiveConstraintGraph*, std::vector<Edge>, CgHasher>
      cgchildren;
