...
PointSet::PointSet(vector<Point *> atoms) { this->points = atoms; }
...
void PointSet::addAtom(Point *a) { this->points.push_back(a); }

bool PointSet::contains(Point *a) {
  return (find(this->points.begin(), this->points.end(), a) !=
          this->points.end());
}

bool PointSet::empty() { return this->points.empty(); }

size_t PointSet::size() { return this->points.size(); }
...
vector<Point *> PointSet::getAtoms() { return this->points; }
...
vector<pair<int, int> > PointSet::getDumbbells() {
  ...
  if (middleDumbbells) {
    minAtom =
        this->points[settings::RootNodeCreation::participatingAtomIndex_low]
            ->getLocation()[mainAxis];
    maxAtom =
        this->points[settings::RootNodeCreation::participatingAtomIndex_high]
            ->getLocation()[mainAxis];
...
void PointSet::buildStree() { this->stree = new Stree(this->points); }
...
Stree *PointSet::getStree() { return this->stree; }
