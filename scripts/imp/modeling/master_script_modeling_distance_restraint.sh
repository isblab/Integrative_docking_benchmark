...
for protein in 1clv_8 1dfj_4 1kxp_7 1r0r_6 2ayo_5 2b42_10 2hle_9; do
    ...
    for i in $(seq 20); do
        folder_name="$i"
        mpirun -np 4 ~/imp-clean/build/setup_environment.sh python ~/Integrative_docking_benchmark/scripts/imp/modeling/sample_imp_distance_restraint.py "prod" "$chainA" "$chainB" "$folder_name" "DMTMM" "20" 2> "err_$folder_name.log" &
    done
