...
##-------------------------------------##
print("\n<-----------PMI Analysis----------->")
subprocess.call( [f"{imp_path}", "python", "~/Integrative_docking_benchmark/scripts/imp/analysis/run_analysis_trajectories_distance_restraint.py", f"{modeling_dir_path}"] )
...
if int( cluster[1] ) >= 30000:
	...
	subprocess.call( ["python", "~/Integrative_docking_benchmark/scripts/imp/analysis/variable_filter_v1.py", "-c", f"{cluster[0]}", "-g", "model_analysis/"] )
	subprocess.call( [f"{imp_path}", "python", "~/Integrative_docking_benchmark/scripts/imp/analysis/run_extract_models_with_vf.py", f"{modeling_dir_path}", f"{cluster[0]}"] )

else:
	subprocess.call( [f"{imp_path}", "python", "~/Integrative_docking_benchmark/scripts/imp/analysis/run_extract_models_without_vf.py", f"{modeling_dir_path}", f"{cluster[0]}"] )
