...
class AnalysisTrajectories(object):
    ...
    def plot_scores_restraints(self, selected_scores, ts_eq, burn_in,
        ...
        axes = ax.flatten()
        selected_scores_array = selected_scores["MC_frame"].values
        score_indices = np.arange(0, len(selected_scores_array),10)
        plot_indices = np.arange(burn_in, len(selected_scores_array), 10)

        column_names = selected_scores.columns.tolist()
        for i, c in enumerate(selected_scores.columns.values[1:]):
            axes[i].plot(
                selected_scores_array[score_indices[:len(plot_indices)]],
                selected_scores[column_names[i]].values[plot_indices],
                color="b",
                alpha=0.5,
            )
    ...
    def extract_models_to_single_rmf(
        ...
        row1 = inf.iloc[0]
        rmf_file = os.path.join(top_dir, row1.rmf3_file)
    ...
    def plot_XLs_satisfaction(self, S_info, ts_max, file_out):
        ...
        XLs_percent = pd.DataFrame()
        for col in S_info.columns:
            if "XLs_satif" in col:
                XLs_percent[col] = S_info[col]


        XLs_nuis = pd.DataFrame()
        for col in S_info.columns:
            if "CrossLinkingMassSpectrometryRestraint_Psi_" in col:
                XLs_nuis[col] = S_info[col]



        # XLs_nuis = S_info["CrossLinkingMassSpectrometryRestraint_Psi_"].values
        # XLs_nuis = S_info.columns.tolist()  # Get a list of column names
        # print(XLs_nuis)
        # exit()
        XLs_percent_list = XLs_percent.columns.tolist()
        XLs_nuis_list = XLs_nuis.columns.tolist()

        XLs_percent_list.sort()
        XLs_nuis_list.sort()

        fig, ax = pl.subplots(figsize=(10.0, 4.0), nrows=1, ncols=3)
        axes = ax.flatten()

        S_info_array = S_info["MC_frame"].values
        nuis_indices = np.arange(0, len(XLs_nuis),20)
        indices = np.arange(0, len(S_info_array),20)
        percent_indices = np.arange(0, len(XLs_percent),20)


        for i, c in enumerate(XLs_percent):
            label = c
            # column_names = XLs_percent.columns.tolist()
            axes[0].plot(
             S_info_array[indices],
             XLs_percent[XLs_percent_list[i]].values[percent_indices],
                         label=label)
        ...
        for i, c in enumerate(XLs_nuis):
            ...
            axes[1].plot(
            S_info_array[indices],
            XLs_nuis[XLs_nuis_list[i]].values[nuis_indices],
                         label=label)
